...
class TestPhorum(SeleniumTestCase):
    ...
    def test_comment_text_rendered(self):
        text = self.selenium.find_element_by_xpath('//div[@id="page-phorum"]//p[@class="comment_text"]').text
        self.assertEquals(self.comment.text, text)

    def test_author_link_rendered(self):
        url = self.selenium.find_element_by_xpath('//div[@id="page-phorum"]//span[@class="nick"]/a').get_attribute('href')
